...
输出报告落盘到工作区草稿目录 `.issues/<YYYY-MM-DD-主题>/niche.md`（目录未被 git 忽略时先写入 `.git/info/exclude`）——核实计划要「跑完两路回来」，落盘就是回来的载体。报告结构：候选描述 / 信号证据（注明每条来源与核实状态）/ 红海测试结果 / 评分卡 / 五要素复核 / **结论四态：go·短窗套利型（必附退出纪律——窗口关闭信号与止损线）、go·长线复利型、收窄重评、no-go** / 下一步建议（go 时建议用 opc-checkup 做完整商业体检）。
