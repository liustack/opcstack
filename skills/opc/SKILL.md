...
路由表里没有一行对得上，就不要硬套技能，直接正常回答用户的问题即可。纯思维方法、开发工作流类的诉求（思维模型、需求规格化、核实纪律）属于姊妹栈 liustack 的对应技能（thinking/clarify/best-practices 等），装有时按其触发规则处理，未装时可提示 `npx -y skills add liustack/liustack`。
