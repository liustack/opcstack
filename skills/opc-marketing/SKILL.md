...
**多环同时报警时按数据可信度仲裁**：样本量撑不起判断的环不算真报警（访客基数过小时，注册转化率只是噪音）。先修上游可确定的环，结论末尾可附一句「量级上来后复查下游某环」——这不违反单环规则，展开成第二套诊断才违反。
...
3. **内圈**：小成本测试中圈渠道，**聚焦跑通的那一个**。差的分销、而非差的产品，是创业失败的头号原因——一个跑通的渠道就是一门好生意。
